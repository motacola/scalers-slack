...
from .utils import iso_to_unix_ts
...
class CommandEngine:
    ...
    def _handle_interactive_login(self, cmd: str) -> str:
        ...
        try:
             ...
             if self.engine.browser_config.headless:
                 return "I'm currently running in headless mode (invisible). Please check config.json and set 'headless': false to use this feature."
    ...
    def _handle_session_check(self, cmd: str) -> str:
        ...
        if "logged in" in cmd and "im" in cmd or "i am" in cmd:
            if self.engine.browser_session:
                try:
                    ...
                    return "Thanks! I've refreshed the browser session to pick up your new login state. You should be good to go."
                except Exception as e:
        ...
        if self.engine.browser_session:
            try:
                ...
                if "google" in cmd:
                     results.append("ℹ️ Google: To verify/login to Google, please run: python scripts/session_manager.py --google --interactive")
            except Exception as e:
    ...
    def _handle_sync(self, cmd: str) -> str:
        ...
        if not projects_to_run:
            return "I couldn't identify which project you want to sync. Try saying 'sync everything' or naming a specific project."
